...
function imprimirResultados(matriz, n_col){
    ...
    var nulidad=matriz[0].length-matriz.length-1;
    var im=matriz[0].length-1-nulidad;
    var arregloAux=new Array(matriz.length);
    if(im>0){
        console.log("Es mayor a cerooooooooooooooooooooooooooooooo "+im);
        for(i=0; i<copiaMatriz.length; i++){
            arregloAux[i]= new Array(im);
            for(j=0; j<arregloAux[i].length; j++){
                arregloAux[i][j]=copiaMatriz[i][j];
            }
        }
        /*
        if(calcularLI(arregloAux)){
            console.log("Aca se breikeaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
            for(i=0; i<arregloAux.length; i++){
                imagen[i]= new Array(arregloAux[i].length-1);
                for(j=0; j<arregloAux[i].length-1; j++){
                    imagen[i][j]=arregloAux[i][j];
                }
            }
            ...
            //console.log("Aqui llegaxd?");
            
        for(h=0; h<im; h++){
            
                //console.log("Aqui llega en teoria");
            console.log(nulidad);
            for(g=im; g<(nulidad+im); g++){
                for(i=0; i<copiaMatriz.length; i++){
                    //console.log("Y aca tambien pero pues xd");
                    arregloAux[i][h]=imagen[i][g];
                    console.log(arregloAux[i][h]);
        ...
        console.log("Aqui Termina el forrrrrrrrrrrrrrr");
    }else{
        console.log("Es menor a cerooooooooooooooooooooooooooooooo");
    ...
    for(j=0;j<arregloAux[0].length;j++){
        baseim += "(";
        baseim += "<tr>";
        for(i=0; i<arregloAux.length; i++){
            /*var expsa = '2x + 3x';
            ...
            */
            if(i!=arregloAux.length-1){
                if(arregloAux[i][j]%1==0){
                    baseim += arregloAux[i][j]+', ';
                }else{
                    dato = math.format(math.fraction(arregloAux[i][j]), { fraction: 'ratio' })
                    baseim += dato+', ';
                }
            }else{
                if(arregloAux[i][j]%1==0){
                    if(j!=arregloAux[i].length-1){
                        baseim += arregloAux[i][j]+'), ';
                    }else{
                        baseim += arregloAux[i][j]+')';
                    }
